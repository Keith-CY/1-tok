...
describe("buyer deposit helpers", () => {
  it("builds a wallet-friendly QR payload and url", () => {
    expect(buildBuyerDepositQRCodeValue(demoDeposit.address)).toBe(`ckb:${demoDeposit.address}`);
    expect(buildBuyerDepositQRCodeURL(demoDeposit.address)).toContain(encodeURIComponent(`ckb:${demoDeposit.address}`));
  });
  ...
  it("renders QR and copy affordances for the buyer deposit address", () => {
    ...
    expect(html).toContain(encodeURIComponent(`ckb:${demoDeposit.address}`));
  });
});
