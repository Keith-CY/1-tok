...
cleanup() {
  ...
  if [[ $code -ne 0 ]]; then
    echo "compose e2e failed; logs are in ${LOG_DIR}" >&2
...
compose up -d --build e2e-runner

runner_id="$(wait_for_container e2e-runner)"
exit_code="$(wait_for_exit "${runner_id}" "e2e-runner")"

if [[ "${exit_code}" != "0" ]]; then
  echo "e2e-runner failed with exit code ${exit_code}" >&2
  exit "${exit_code}"
fi
